        ...
        <dl className="mt-6 space-y-4">
            {
              order.orderProducts.map(product => (
                <div 
                  key={product.id} 
                  className="flex item-center gap-2 border-t border-grey-200 pt-4">
                    <dt className="flex item-center text-sm text-grey-600">
                      <span className="font-black">({product.quantity}){''}</span>
                    </dt>
                    <dd className="tetx-sm font-medium text-grey-900">{product.product.name}</dd>
                  </div>
              ))
            }
